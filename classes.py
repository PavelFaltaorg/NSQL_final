...
class Entity:
    ...
    def update_position(self, dt: float, fps: int, server_fps: int):
        ...
        # elif len(self.positions_buffer) == 1:
        #     # Handle fallback to extrapolation if buffer has only one position
        #     extrapolated_position = self.positions_buffer[0][1] + self.velocity * dt
        #     self.shape.position = extrapolated_position
        #     self.u = 0  # Reset u during extrapolation


        # # If only one position in the buffer, fallback to extrapolation
        # elif len(self.positions_buffer) == 1:
        #     extrapolated_position = self.positions_buffer[0][1] + self.velocity * dt
        #     self.shape.position = extrapolated_position
        #     self.u = 0  # Reset u during extrapolation
